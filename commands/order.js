...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((sub) =>
            sub
                .setName('status')
                .setDescription('Check the status of a logged order.')
                .addIntegerOption((o) => o.setName('order_id').setDescription('Order ID').setRequired(true))
        )
        .addSubcommand((sub) =>
            sub
                .setName('update')
                .setDescription('Update a logged order\'s status (staff only).')
                .addIntegerOption((o) => o.setName('order_id').setDescription('Order ID').setRequired(true))
                .addStringOption((o) =>
                    o
                        .setName('status')
    ...
    async execute(interaction) {
        ...
        if (sub === 'status') {
            ...
            const order = orders.findOrder(guildId, id);
            if (!order) return interaction.reply({ content: `No order found with ID \`#${id}\`.`, ephemeral: true });
            return interaction.reply({ content: `Current status for order \`#${id}\`: **${order.status}**`, embeds: [orderEmbed(cfg, order)], ephemeral: true });
        }
