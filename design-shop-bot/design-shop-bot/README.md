...
This is a separate system from `/order log|status|view|history|reset`,
...
`/order status` sets Paid/Void/etc. and replies with the full order
embed. `/order view`, `/order history`, `/order reset`, and
