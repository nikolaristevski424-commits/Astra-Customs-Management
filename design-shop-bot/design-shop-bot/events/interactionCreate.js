...
async function handleButton(interaction) {
    ...
    if (customId.startsWith('ticket_claim_')) {
        ...
        tickets.update(guildId, channelId, { claimedBy: interaction.user.id });
...
async function handleModal(interaction) {
    ...
    if (customId === 'ticket_open_modal') {
        ...
        tickets.create(guildId, channel.id, { userId: interaction.user.id, reason });
        ...
        await channel.send({
            content: `<@${interaction.user.id}>${cfg.staffRoleIds[0] ? ` <@&${cfg.staffRoleIds[0]}>` : ''}`,
            embeds: [{ title: 'New Ticket', description: reason, color: 0x2d2d31 }],
            components: [row],
            allowedMentions: { users: [interaction.user.id], roles: cfg.staffRoleIds.slice(0, 1) },
        });
